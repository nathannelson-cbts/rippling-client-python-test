...
**ALWAYS use the temp file method for git commits — NEVER pass commit messages via terminal commands.**

Passing commit messages through the shell causes terminal hangs and escaping failures. The ONLY safe method:
...
2. **Commit using the file**:
   ...
   ```
3. **Clean up** (required — prevents stale messages in future commits):
   ```bash
...
**NEVER do this:**
```bash
# WRONG - all of these have escaping risks or cause hangs
git commit -m "message"
git commit -m "line 1
line 2"
echo "message" | git commit -F -
printf 'message' > .git/COMMIT_MSG_TEMP
cat > .git/COMMIT_MSG_TEMP << 'EOF'
